...
function generarPropuestaPDF(u) {
  ...
  doc.text('Creado por JMLL', W / 2, H - 0.9, { align: 'center' });
...
async function compartirPropuestaPDF(u, generar = generarPropuestaPDF) {
  try { await Promise.all([cargarJsPDF(), cargarLogoPDF(), generar === generarFRM106PDF ? cargarFondoFRM106() : null]); }
  catch (e) { toast(e.message + '. Revisa tu conexión.'); return; }
...
/* ---------- FRM-CR106: Autorización para investigación de antecedentes ----------
   Se usa la hoja original del banco como fondo (icons/frm-cr106.jpg, tamaño carta) y encima se
   escriben la ciudad, la fecha de la propuesta y el nombre completo y CI del titular y del codeudor,
   en la posición de cada campo. El cliente lo imprime, firma y escanea. */
let fondoFRM106 = null;
function cargarFondoFRM106() {
  if (fondoFRM106) return Promise.resolve(fondoFRM106);
  return fetch('icons/frm-cr106.jpg').then(r => { if (!r.ok) throw new Error('No se pudo cargar el formulario'); return r.blob(); })
    .then(b => new Promise(ok => { const fr = new FileReader(); fr.onload = () => ok(fondoFRM106 = fr.result); fr.readAsDataURL(b); }));
}
function generarFRM106PDF(u) {
  const { jsPDF } = window.jspdf;
  const doc = new jsPDF({ unit: 'pt', format: 'letter' });   // 612 × 792 pt, igual que el original
  const V = u.V;
  doc.addImage(fondoFRM106, 'JPEG', 0, 0, 612, 792);
  doc.setTextColor(0, 0, 0);
  // Ciudad y fecha (día/mes/año), alineadas a la derecha como en el original
  const [a, m, d] = today().split('-');
  doc.setFont('helvetica', 'bolditalic'); doc.setFontSize(9.5);
  doc.text(`${(V.ciudadFrm || 'La Paz').trim()}, ${d}/${m}/${a}`, 526.7, 183.6, { align: 'right' });
  // Firmas: titular arriba a la izquierda, codeudor arriba a la derecha
  const firmantes = [{ n: V.nombre, ci: V.ci, ext: V.ext }];
  if (V.codeudor === 'si') firmantes.push({ n: V.cNombre, ci: V.cCi, ext: V.cExt });
  const campos = [{ xn: 130, xc: 102, yn: 620.6, yc: 634.1, max: 172 }, { xn: 352, xc: 324, yn: 620.6, yc: 634.1, max: 172 }];
  firmantes.forEach((p, i) => {
    const c = campos[i];
    const nombre = String(p.n || '').trim().toUpperCase();
    doc.setFont('helvetica', 'bold');
    let tam = 9.5; doc.setFontSize(tam);
    while (tam > 6.5 && doc.getTextWidth(nombre) > c.max) { tam -= 0.5; doc.setFontSize(tam); }
    if (nombre) doc.text(nombre, c.xn, c.yn);
    doc.setFontSize(9.5);
    if (p.ci) doc.text(`${p.ci}${p.ext ? ' ' + p.ext : ''}`, c.xc, c.yc);
  });
